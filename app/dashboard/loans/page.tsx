...
export default function LoanRequestPage() {
  ...
  if (loading) {
    return <p className="text-center mt-10">Checking eligibility…</p>;
  }

  const eligible = balance >= MIN_BALANCE;

  return (
    <div className="max-w-xl mx-auto pt-6 h-screen space-y-6">
      ...
      >
        ...
        {!eligible && (
          <p className="mt-3 text-sm text-red-700">
            You must deposit at least ${MIN_BALANCE} to apply for a loan.
          </p>
        )}
      ...
      {eligible ? (
        <form className="bg-white p-6 rounded-xl shadow space-y-4">
          ...
          <button className="w-full bg-[var(--headtext)] text-white py-3 rounded-lg">
            Submit Loan Request
          </button>
        </form>
      ) : (
        ...
        <button
          className="w-full bg-[#0D1F17] text-gray-400 py-3 rounded-lg "
        >
          Deposit $500 to Unlock Loan Application
        </button>
        </Link>
      )}
    </div>
